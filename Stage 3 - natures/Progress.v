...
Lemma single_threaded_progress :
  forall P t' Gamma H V n Ls e t mcs n_nature ncs, (*!!*)
    wfProgram P t' mcs -> (*!!*)
...
Proof with eauto using step.
  ...
  hasType_cases(induction hasType) Case;
    ...
    repeat
    match goal with
      | [H : Types.hasType ?P ?Gamma (EVar (DV ?x)) _ _ _|- _ ] => (*!!*)
        inv H
      | _ => idtac
    end;
  ...
  + Case "T_Call".
    ...
    - SCase "e done".
      ...
      assert (methods P (TClass c0) = Some ms)  (*!!*)
        by (simpl; rewrite cLookup; eauto).
      eexists; eapply EvalCall...
    - SCase "e can step".
      destruct eSteps as [[[[H' V'] nat'] T'] eSteps].  (*!!*)
      destruct T'...
      inv eSteps...
      inv eSteps...
  + Case "T_Select".
    right. 
    inv hasType...
    assert (TClass c0 = TClass c)...
    wfEnvLookup.
    eapply dyn_wfFieldLookup in wfF as [v [nature [ ncs []]]]...  (*!!*)
...
Theorem progress :
  forall P t' Gamma cfg t mcs n ncs,
    wfProgram P t' mcs -> (*!!*)
    wfConfiguration P Gamma cfg t n ncs ->
...
Proof with eauto.
  ...
  + Case "T = Async T1 T2 e".
    ...
    right. right.
    pose proof (IHT1 wfL1 t1 n2 ncs1 wfT1) as IH1. (*!!*)
    pose proof (IHT2 wfL2 t2 n3 ncs2 wfT2) as IH2. (*!!*)
