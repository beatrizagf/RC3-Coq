...
Lemma wfProgram_wfMethodDecl :
  forall P t' c ms m mtd mcs_p,
    wfProgram P t' mcs_p -> (*!!*)
    methods P (TClass c) = Some ms ->
    methodLookup ms m = Some mtd ->
exists mcs_m,
    wfMethodDecl P c mcs_m mtd.
Proof with auto.
  ...
  assert (Heq : c = c') by
      (unfold classLookup in HeqcLookup;
       ...
       apply eqb_class_id_eq; auto).    (*change beq_nat_eq to eqb_class_id_eq*)
  subst.
...
Lemma methodSigs_sub :
  forall P t t1 t2 m msigs1 msigs2 msig mcs,
    wfProgram P t mcs -> (*!!*)
    subtypeOf P t1 t2 ->
...
Proof with eauto using
  ...
  + rename msigs2 into msigs3.
    ...
    assert (wfT1: wfType (cds, ids, e) t1).   (*!!*)
      simple eapply subtypeOf_wfTypeSub.
      apply WF_Program with (n := n)...
      exact Hsub1.
    assert (wfT2: wfType (cds, ids, e) t2). (*!!*)
      simple eapply subtypeOf_wfTypeSup.
      apply WF_Program with (n := n)...
      exact Hsub1.
...
Lemma wfFields_declsToFields :
  forall P t' c i fs ms Gamma mcs,
    wfProgram P t' mcs -> (*!!*)
    wfEnv P Gamma ->
...
Proof with eauto using
  ...
  econstructor...
  intros.
  lookup_forall as wfF. inv wfF. exists... (*!!*)
Qed.

Lemma wfFields_extend :
  forall P Gamma c fs f t F v n ncs, (*!!*)
    fields P (TClass c) = Some fs ->
    fieldLookup fs f = Some (Field f t) ->
    wfFields P Gamma c F ->
...
Proof with eauto using hasType_extend_loc.
  ...
  econstructor...
  introv Hlookup.
  apply wfFlds in Hlookup as (v & n & ncs & Heq & hasType).
  exists...
Qed.

Lemma wfFields_invariance :
  forall P t' c Gamma Gamma' F mcs,
    wfProgram P t' mcs -> (*!!*)
...
Proof with eauto using hasType_wfType.
  ...
  econstructor...
  introv fLookup.
  apply wfFld in fLookup as (v & n & ncs & Ff & hasType). (*!!*)
  exists v. exists. split... (*!!*)
  + inv hasType...
    econstructor...
    rewrite <- envSub...
Qed.
...
Lemma wfVars_invariance :
  forall P t' Gamma Gamma' fsyms V mcs,
    wfProgram P t' mcs -> (*!!*)
...
Proof with eauto.
  ...
  + introv Vlookup.
    ...
    apply envModelsVars in Vlookup as (v & n & ncs & Vlookup & hasType). (*!!*)
    eapply hasType_subsumption with (Gamma' := Gamma') in hasType; crush...
  + introv. rewrite <- envEquiv...
Qed.

Lemma wfVars_extend :
  forall P t' Gamma n m V v t mcs n' ncs',
    wfProgram P t' mcs -> (*!!*)
    wfVars P Gamma n V ->
    P; Gamma |- EVal v \in t # n' |> ncs' ->  (*!!*)
    m < n ->
...
Proof with eauto with env.
  ...
  + introv envLookup.
    ...
    - subst. simpl_extend_hyp.
      inv_eq.
      exists v. 
      inv hasType; exists; split... (*!!*)
    - rewrite extend_neq in envLookup...
      apply envModelsVars in envLookup as (v' & nature & NCS & Vlookup & hasType'). (*!!*)
      exists v'.
      inv hasType'; exists; split... (*!!*)
...
Corollary wfThreads_wfEnv :
  forall P t' Gamma T t mcs n ncs,
    wfProgram P t' mcs -> (*!!*)
...
Lemma wfThreads_invariance :
  forall P t' Gamma Gamma' T t mcs n ncs,
    wfProgram P t' mcs -> (*!!*)
    (forall x, Gamma x = Gamma' x) ->
    wfThreads P Gamma T t n ncs -> (*!!*)
    wfThreads P Gamma' T t n ncs. (*!!*)
...
Lemma wfThreads_heapExtend :
  forall P t' Gamma T t c l mcs n ncs,
    wfProgram P t' mcs -> (*!!*)
    wfType P (TClass c) ->
    fresh Gamma (env_loc l) ->
    wfThreads P Gamma T t n ncs ->  (*!!*)
    wfThreads P (extend Gamma (env_loc l) (TClass c)) T t n ncs. (*!!*)
Proof with eauto using hasType_extend_loc with env.
  introv wfP wfTy Hfresh wfT.
  generalize dependent t. gen n ncs. (*!!*)
...
Lemma wfConfiguration_substitution :
  forall P Gamma H V n Ls e e' t n' n'' ncs' ncs'',
    freeVars e' = nil ->
    P; Gamma |- e' \in t # n' |> ncs' -> (*!!*)
    wfLocking H (T_Thread Ls e') ->
    wfConfiguration P Gamma (H, V, n, T_Thread Ls e) t n'' ncs''-> (*!!*)
    wfConfiguration P Gamma (H, V, n, T_Thread Ls e') t n' ncs'. (*!!*)
Proof with eauto.
...
Lemma wfConfiguration_heapUpdate :
  forall P t' Gamma H V n T t l c F L L' F' mcs n' ncs',
    wfProgram P t' mcs -> (*!!*)
    wfConfiguration P Gamma (H, V, n, T) t n' ncs'->  (*!!*)
    heapLookup H l = Some (c, F, L) ->
    wfFields P Gamma c F' ->
    ...
    wfConfiguration P Gamma
                    ((heapUpdate H l (c, F', L')), V, n, T) t n' ncs'.  (*!!*)
Proof with eauto using
                 wfHeap_update,
