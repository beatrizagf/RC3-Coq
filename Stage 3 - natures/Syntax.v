...
Definition FieldsType (f : fieldDecl) : ty := 
  match f with
      | Field _ t => t
  end.
...
Inductive nature : Set :=
  | NAtomic : nature      (*so it has a different name from atomicity*)
  | NNonAtomic : nature
  | Undefined : nature
  | NVar : var -> nature
...
(*to transform into atomic*)
Definition addAtomic (t : ty) : ty:=
  match t with
    | TClass (c, a) => TClass (c, Atomic)
