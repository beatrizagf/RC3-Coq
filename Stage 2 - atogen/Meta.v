...
Definition class_id := prod nat atomicity.
Definition interface_id := prod nat atomicity.
Definition loc := nat.
Definition lock := loc%type.


Definition atogenClId (x: class_id) := (fst x, Atomic). (*to generate atomic*)
Definition atogenInId (x: interface_id) := (fst x, Atomic). (*to generate atomic*)
...
Proof.
  ...
  intros. apply andb_true_eq in H. (*separar &&*)
  destruct H as [H1 H2]. (*to separate the hypothesis*)
  apply beq_nat_eq in H1.
  apply eqb_atomicity_eq in H2.
  rewrite H1. rewrite H2. reflexivity.
