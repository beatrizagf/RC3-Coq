...
(**************atogen function for list fields*******************)

Fixpoint atogenfield(c : class_id)(i : interface_id)(fs : list fieldDecl) := 
 match fs with
    | nil => nil
    | h :: t => match h with 
                          | Field f (TClass c') => match eqb_class_id c c' with
                                                                              | true => Field f (TClass (atogenClId c))   (*if field has type class, with the same class_id as the class_id received, then class becomes atomic*)
                                                                              | false =>  Field f (TClass c')    (*if the class is diferent, then it stays the same*)
                                                   end
                          | Field f (TInterface i') => match eqb_interface_id i i' with
                                                                              | true => Field f (TInterface (atogenInId i)) (*if field has type interface, with the same interface_id as the interface_id received, then interface becomes atomic*)
                                                                              | false =>  Field f (TInterface i')  (*if the interface is diferent, then it stays the same*)
                                                        end
                          | Field f TUnit => Field f TUnit
                end :: atogenfield c i t
  end. 
(**********************************)
...
(**************atogen function for list class*******************)
Fixpoint atogenCl(cds : list classDecl) : list classDecl := 
  match cds with
    | nil => nil
    | h :: t => match h with 
                          | Cls c i f l0 => Cls (atogenClId c) (atogenInId i) (atogenfield c i f) l0   (*class and interface becomes atomic*)
                end :: atogenCl t
  end.
(**********************************)
...
(**************atogen function for list interface*******************)
Fixpoint atogenIn(ids : list interfaceDecl) : list interfaceDecl :=
 match ids with
    | nil => nil
    | h :: t => match h with
                           | Interface i l => Interface (atogenInId i) l   (*interface becomes atomic*)
                           | ExtInterface i i0 i1 => ExtInterface (atogenInId i) (atogenInId i0) (atogenInId i1) 
                end :: atogenIn t
  end.
(**********************************)
...
(**************atogen function for pogram*******************)
Definition atogenProg(P : program) :=
  match P with
    | (cs, ids, e) => ((cs ++ atogenCl cs), (ids ++ atogenIn ids), e)
  end.
(**********************************)
